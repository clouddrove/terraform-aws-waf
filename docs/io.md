...
| bucket\_key\_enabled | Whether to enable S3 bucket key for SSE-KMS encryption | `bool` | `false` | no |
...
| deletion\_protection\_enabled | Whether to enable deletion protection for CloudWatch Log Groups | `bool` | `false` | no |
...
| multi\_region | Whether to create a multi-region KMS key | `bool` | `false` | no |
...
| region | AWS region to use for regional resources | `string` | `""` | no |
...
| web\_acl\_arn | The ARN of the WAFv2 Web ACL. Use this to associate the ACL with an ALB, CloudFront, or API Gateway. |
